...
decimalCeiling <- function(func_value, func_level=0){ 
	...
	func_value <- strsplit(as.character(func_value),'.',fixed=TRUE)
	return (
		unlist(lapply(func_value, function(funcSet){
			...
			if(length(funcSet) > 1){
				...
				func_deciList <- c(as.numeric(funcSet[1]),as.numeric(strsplit(funcSet[2],"")[[1]]))
